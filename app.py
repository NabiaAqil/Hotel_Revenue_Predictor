...
city = st.selectbox('Select City:', ['Islamabad', 'Lahore', 'Karachi', 'Other'])
...
is_Islamabad = 1 if city == "Islamabad" else 0
is_BookingCom = 1 if booking_source == "Booking.com" else 0
is_TravelAgency = 1 if booking_source == "Travel Agency" else 0
is_WalkIn = 1 if booking_source == "Walk-in" else 0
is_Website = 1 if booking_source == "Website" else 0
...
if st.button("Predict Hotel Revenue 🔮", use_container_width=True):
    user_data = pd.DataFrame([{
        'Month': month, 'Avg_Occupancy_%': avg_occupancy, 'Room_Rate_PKR': room_rate,
        'Total_Rooms': total_rooms, 'Staff_Count': staff_count, 'Amenities_Count': amenities_count,
        'Customer_Rating': customer_rating, 'Avg_Stay_Duration_Days': avg_stay,
        'Booking_Source_Booking.com': is_BookingCom, 'Booking_Source_Travel Agency': is_TravelAgency,
        'Booking_Source_Walk-in': is_WalkIn, 'Booking_Source_Website': is_Website,
        'City_Islamabad': is_Islamabad, 'Province_Balochistan': is_Balochistan,
        'Province_KPK': is_KPK, 'Province_Punjab': is_Punjab, 'Province_Sindh': is_Sindh,
        'Season_Autumn': is_Autumn, 'Season_Spring': is_Spring, 'Season_Summer': is_Summer, 'Season_Winter': is_Winter
    }])
    ...
    try:
        ...
        st.error(f"Columns alignment issue: {e}")
